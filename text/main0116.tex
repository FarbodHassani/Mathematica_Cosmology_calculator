...
$. The well-known relations with the red-shift $z$ are explained in
...
on another parameter called red-shift and it is defined as  $\bm
z(\tau) \equiv 1/\atau -1$ and the conformal time for a given red-shift is
...
$\tau $, it is common to determine epochs in terms of the red-shift $z$.
...
The
advantage of using red-shift is that  we can control the initial
red-shift  $z_{\rm i}$ and final red-shift $z_{\,\rm f}$. Because we
...
\ee 
Here for simplicity we neglect radiation contribution which is consistent with the fact that we take the initial red-shift in the matter dominated era,  therefore
\be
